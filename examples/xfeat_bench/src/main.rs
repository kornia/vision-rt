...
use vrt::{Engine, Logger, Runtime, Pipeline, Source, PipelineTiming, VrtImage, Format, MemKind};
...
impl Source for StaticImageSource {
    ...
    fn next_frame(&mut self) -> Option<VrtImage> {
        ...
        Some(unsafe {
            VrtImage::borrowed(self.ptr, SRC_W, SRC_H, SRC_W * 4, Format::Rgba8, MemKind::Device)
        })
    }
}
...
fn main() -> Result<(), vrt::BoxError> {
    ...
    let xfeat   = XFeat::with_stream(Arc::clone(&engine), stream.clone(),
    ...
    let resized = image::imageops::resize(&img, SRC_W, SRC_H, image::imageops::FilterType::Triangle);
    let mut rgba = Vec::with_capacity((SRC_W * SRC_H * 4) as usize);
    for px in resized.pixels() { rgba.extend_from_slice(&[px[0], px[1], px[2], 255]); }
    let backing: CudaSlice<u8> = stream.memcpy_stod(&rgba)?;
    let ptr = backing.device_ptr(stream.as_ref()).0 as *mut c_void;

    let source = StaticImageSource { ptr, remaining: WARMUP + iters, _backing: backing };
    ...
    while let Some(res) = pipeline.next() {
        let (kpts, t) = res?;
        n += 1;
        if n <= WARMUP { continue; }   // discard warm-up (TRT/CUDA cache fill)
    ...
    println!("  source   {:.3}   (synthetic — exclude)", mean(acc.source_ms));
    println!("  enqueue  {:.3}   (CPU kernel/TRT launch)", mean(acc.enqueue_ms));
    println!("  gpu      {:.3}   (CUDA events: letterbox + backbone + top-K)", mean(acc.gpu_ms));
    ...
    println!("  mean {:.2}   p50 {:.2}   p99 {:.2}   min {:.2}   max {:.2}",
        total_mean, pct(&mut totals, 0.50), pct(&mut totals, 0.99),
    ...
    println!("\n  gpu-only: mean {:.2}   p50 {:.2}   p99 {:.2} ms",
        gpu.iter().sum::<f64>() / m, pct(&mut gpu, 0.50), pct(&mut gpu, 0.99));
    Ok(())
}
