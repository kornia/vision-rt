...
fn main() {
    ...
    // ── 3. autocxx: generate Rust bindings from TRT C++ headers ────────────────────────
    // autocxx reads src/lib.rs for the include_cpp! macro and generates
    // the Rust+C++ glue code from the TRT headers it finds.
    // This is the key change from the hand-written shim approach: when TRT updates,
    // you get Rust compile errors pointing exactly to what changed.
    let mut autocxx_build = autocxx_build::Builder::new(
            "src/lib.rs",
            [&trt_inc as &str, &cuda_inc, "include"],
        )
        .extra_clang_args(&[
            "-std=c++17",
        ...
        .build()
        .expect("autocxx build failed on TRT headers — see BLOCKED note in README");
    ...
    // ── 4. bindgen for the btrt_* C bridge (logger + runtime/engine/context + CUDA) ────
