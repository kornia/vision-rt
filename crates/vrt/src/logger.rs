use std::sync::Arc;
use std::ffi::CStr;
use vrt_sys::{btrt_logger_t, btrt_logger_create, btrt_logger_destroy, btrt_logger_set_callback};

/// TRT log severity (mirrors ILogger::Severity).
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub enum Severity { InternalError = 0, Error = 1, Warning = 2, Info = 3, Verbose = 4 }
...
extern "C" fn rust_log_callback(severity: i32, msg: *const std::os::raw::c_char) {
    ...
    let msg_str = if msg.is_null() { "" } else {
...
impl Logger {
    ...
    pub fn new(min_severity: Severity) -> crate::error::Result<Arc<Self>> {
        ...
        unsafe { btrt_logger_set_callback(ptr, Some(rust_log_callback)); }
        Ok(Arc::new(Self { ptr }))
    }

    pub(crate) fn as_ptr(&self) -> *mut btrt_logger_t { self.ptr }
}
...
impl Drop for Logger {
    fn drop(&mut self) {
        ...
        unsafe { btrt_logger_destroy(self.ptr); }
    }
}
