...
fn main() -> Result<(), vrt::BoxError> {
    ...
    if a.len() < 4 {
        eprintln!("Usage: eval_oxford <dataset_dir> <raco.engine> <lightglue.engine> [inlier_px] [min_cossim]");
        std::process::exit(1);
    }
    ...
    println!(
        "{:<14} {:>26}   {:>26}",
        "", "LightGlue+ (matches/inl/%)", "mutual-NN (matches/inl/%)"
    );

    let manifest = std::fs::read_to_string(root.join("manifest.txt"))?;
    let (mut lg_tot, mut mnn_tot) = (0usize, 0usize);
    ...
    println!("\ntotal correct correspondences: LightGlue+ {lg_tot}, mutual-NN {mnn_tot}");
    Ok(())
}
